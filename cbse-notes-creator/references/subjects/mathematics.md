...
| Component | Weight |
|-----------|--------|
| Worked examples + practice problems | 60% |
| Concept explanation with intuition | 20% |
| Exam prep (model answers, revision) | 20% |

### Formatting Rules
- **Formulas**: Use code blocks. Example: `Area = √[s(s-a)(s-b)(s-c)]` where `s = (a+b+c)/2`
- **Proofs/Derivations**: Step-by-step with reasoning at each step. Number each step.
- **Geometric figures**: Describe in text ("Draw triangle ABC where AB = 5 cm, angle B = 60°, BC = 4 cm")
- **Worked examples**: Follow this pattern: Given → To Find → Formula → Substitution → Solution → Answer (boxed)
- **Practice problems**: 5 per sub-section, graded easy (2) → medium (2) → hard (1). Answers at end of notes.

### Target Word Count
5K-7K words

### Common Pain Points
- **Sign mistakes during substitution**: Students apply formulas correctly but flip signs with negative numbers. Happens in algebra, coordinate geometry, and linear equations. Emphasize bracket discipline and sign-check steps. Happens in Number Systems, Linear Equations, Coordinate Geometry chapters.
- **Formulas memorized but don't know which one to use**: Students can list formulas but pick the wrong one for the problem. Happens in mensuration, algebra, and polynomials. Give a decision rule: "If the question asks for X, use Y because Z." Happens in Heron's Formula, Quadrilaterals, Surface Areas and Volumes chapters.
...
## Exam Patterns — Mathematics

**Paper structure**: Section A (20 MCQs × 1 mark), Section B (6 × 2 marks), Section C (6 × 3 marks), Section D (4 × 5 marks), Section E (3 case-based × 4 marks)
**Total**: 80 marks
**Special question types**: Constructions (with steps), proofs, graph-based questions
**Chapter weightage**: Number Systems (8), Algebra (17), Coordinate Geometry (4), Geometry (28), Mensuration (13), Statistics & Probability (10)

**Sample 1-mark**: What is the decimal expansion of 1/11? → (a) Terminating (b) Non-terminating repeating ✓ (c) Non-terminating non-repeating (d) None
**Sample 2-mark**: Find the value of x if √x = 7. → Squaring both sides: (√x)² = 7² → x = 49
**Sample 3-mark**: Find three rational numbers between 3/5 and 4/5. → Multiply numerator and denominator by 4: 12/20 and 16/20. Three rational numbers: 13/20, 14/20, 15/20.
**Sample 5-mark**: Prove that √2 is irrational. → [Proof by contradiction: assume √2 = p/q in lowest terms, show both p and q must be even, contradicting lowest terms assumption]
